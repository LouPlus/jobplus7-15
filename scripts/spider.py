...
class FakerData(object):
    ...
    def fake_job(self):
        companies = User.query.filter_by(role=User.COMPANY).all()
        for i in range(100):
            company = random.choice(companies)
            job = Job(
                name=fake.word() + '工程师',
                low=random.randrange(3000, 8000, 1000),
                high=random.randrange(8000, 20000, 1000),
                location=company.detail.location,
                tags=','.join([fake.word() for i in range(3)]),
                company=company,
                experience=random.choice(['不限', '1', '1-3', '3-5', '5+']),
                degree=random.choice(['不限', '本科', '硕士', '博士']),
            )
            db.session.add(job)
            db.session.commit()
